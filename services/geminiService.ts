...
if (API_KEY) {
  try {
    ...
    console.warn("Gemini API 초기화 실패:", error);
  }
} else {
  console.warn("GEMINI_API_KEY가 설정되지 않았습니다. 전공 추천 기능을 사용하려면 .env 파일에 VITE_GEMINI_API_KEY를 추가해주세요.");
}

export const getMajorRecommendation = async (interests: string): Promise<string> => {
  if (!ai) {
    return "Gemini API 키가 설정되지 않았습니다. .env 파일에 VITE_GEMINI_API_KEY를 추가해주세요.";
  }
  try {
